...
def get_severity(severity_type, data_row):
    ...
    for data_element in data_row:
        try:
            ...
            if return_value == 'missing':
                return_value = value
            elif (severity_type == "max" and value > return_value) or (severity_type == "min" and value < return_value):
                return_value = value
        except ValueError:
            continue
...
def incorporate_new_adir_features_to_aggregations(in_df, old_features, new_features):
    """ new_features_list is a list of new adir features you might want to add to an existing set of features (old_features)
    ...
    out_df['adir_min_severity'] = out_df[all_base_features].apply(lambda row: get_severity("min", row), axis=1)
...
def run_new_feature_importance_tally(in_df, for_sure_features, new_candidate_features, number_of_tries,
    ...
    model, encoded_features_output, y_predicted_without_dunno, y_predicted_with_dunno,y_predicted_probs = \
        all_data_model(passing_tally_dataset, passing_tally_all_features, feature_encoding_map, outcome_column,
                       sample_weights, dunno_range, RandomForestClassifier, n_estimators=n_estimators,
                       criterion=criterion, max_features=max_features, max_depth=max_depth, class_weight=class_weight)
...
def bootstrap_cross_validate_new_features(data_df, new_features_to_try, for_sure_features, model_parameters_dict,
    ...
    def sampling_function_per_try(dataset):
        return subsample_per_class(dataset, outcome_column, {'autism': bootstrapping_sample_percent, 'not': bootstrapping_sample_percent})
    ...
    for feature in ['']+list(new_features_to_try):
        ...
        def ml_function_per_try(dataset_per_try):
            ...
            if model_parameters_dict.get('young_sample', False):
                metrics = cross_validate_model_with_addon(dataset_per_try, sample_weights_per_try,
                                                          all_feature_columns_for_try, feature_encoding_map,
                                                          outcome_column, dunno_range, n_folds, outcome_classes,
                                                          outcome_class_priors, RandomForestClassifier,
                                                          criterion=criterion, max_features=max_features,
                                                          max_depth=max_depth, n_estimators=n_estimators)
            else:
                metrics = cross_validate_model(dataset_per_try, sample_weights_per_try, all_feature_columns_for_try,
                                               feature_encoding_map, outcome_column, dunno_range, n_folds,
                                               outcome_classes, outcome_class_priors, RandomForestClassifier,
        ...
        averaged_metrics, averaged_metrics_err = bootstrap(test_dataset, bootstrapping_number_of_tries,
                                                           sampling_function_per_try, ml_function_per_try,
                                                           return_errs=True, verbose=False)
        print ''
        print '~~~~~~~~~~~~~~'
