...
def run_xvalidation_agreement(n_expts, dataset, sample_weights, feature_columns, feature_encoding_map, outcome_column, dunno_range,
    ...
    for iexpt in range(n_expts):
        ...
        x_valid_good = dataset.index.isin(output['correctly_predicted_sample_indices']).astype(int)
        if iexpt == 0:
            n_matching_arr = x_valid_good
        else:
            n_matching_arr += x_valid_good
    if verbose:
...
def run_inconclusive_model_pseudo_experiments(n_expts, n_xvalid_expts, frac_holdout, reliability_cutoff, dataset,
    ...
    for iexpt in range(n_expts):
        ...
        reliability_df['XValid_matches'] = run_xvalidation_agreement(n_expts=n_xvalid_expts, dataset=reliability_df,
...
def visualize_experiment_results(experiment_results_df, title, override_columns_to_visualize=None):
    ...
    plt.grid(True)
    x_vals = np.arange(len(ordered_mean_vals.index))
    plt.bar(x_vals, ordered_mean_vals.values)
